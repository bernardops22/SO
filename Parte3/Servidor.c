...
void limpar_lista_consultas (){
  mudar_semaforo ( 0 );
  shm_id = shmget ( KEY, NCONSULTAS * sizeof( Consulta ) + sizeof ( int )*NCONTADORES, IPC_CREAT | 0666 );        //CRIAR SHARED MEMORY
...
void desligar_servidor (){
  ...
  exit_on_error ( mq_status, " - Erro ao terminar a consulta" );
  exit_on_null ( mem_cont, " - Erro ao ligar a memoria partilhada" );
